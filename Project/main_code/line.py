...
class line_cam():
    def __init__(self):
        self.cap = cv2.VideoCapture('line.mp4')
        self.fit_result, self.l_fit_result, self.r_fit_result, self.L_lane,self.R_lane = [], [], [], [], []
...
#out = cv2.VideoWriter('output.mp4', fourcc, 20.0, ( 960, 540 ))
    ...
    def draw_circle(self, img,lines, color=[0, 0, 255]):
        for line in lines:
    ...
    def get_random_samples(self, lines):
    ...
    def compute_model_parameter(self, line):
        # y = mx+n
    ...
    def compute_distance(self, par, point):
        # distance between line & point
    
        return np.abs(par[0]*point[:,0]+par[1]*point[:,1]+par[2])/np.sqrt(par[0]**2+par[1]**2)

    def model_verification(self,par, lines):
        # calculate distance
        distance = self.compute_distance(par,lines)
    ...
    def draw_extrapolate_line(self, img, par,color=(0,0,255), thickness = 2):
    ...
    def draw_fitline(self,img, result_l,result_r, color=(255,0,255), thickness = 10):
        # draw fitting line
        ...
        final = self.weighted_img(lane, img, 1,0.5)  
        return final

    def erase_outliers(self,par, lines):
    ...
    def ransac_line_fitting(self,img, lines, min=100):
        #global fit_result, l_fit_result, r_fit_result
        best_line = np.array([0,0,0])
        if(len(lines)!=0):                
            for i in range(30):           
                sample = self.get_random_samples(lines)
                parameter = self.compute_model_parameter(sample)
                cost = self.model_verification(parameter, lines)                        
                if cost < min: # update best_line
                    min = cost
                    best_line = parameter
                if min < 3: break
                # erase outliers based on best line
            filtered_lines = self.erase_outliers(best_line, lines)
            self.fit_result = self.get_fitline(img, filtered_lines)
        else:
            if (self.fit_result[2]-self.fit_result[1])/(self.fit_result[1]-self.fit_result[0]) < 0:
                self.l_fit_result = self.fit_result
                return self.l_fit_result
            else:
                self.r_fit_result =self.fit_result
            return self.r_fit_result

        if (self.it_result[2]-self.fit_result[1])/(self.fit_result[1]-self.fit_result[0]) < 0:
            self.l_fit_result = self.fit_result
            return self.l_fit_result
        else:
            self.r_fit_result = self.fit_result
    ...
    def detect_lanes_img(self,img):

        height, width = img.shape[:2]

        # Set ROI
        vertices = np.array([[(50,height),(width/2-45, height/2+60), (width/2+45, height/2+60), (width-50,height)]], dtype=np.int32)
        ROI_img = self.region_of_interest(img, vertices)
    
        # Convert to grayimage
        #g_img = grayscale(img)
           
        # Apply gaussian filter
        blur_img = self.gaussian_blur(ROI_img, 3)
        ...
        canny_img = self.region_of_interest(canny_img, vertices2)

        # Perform hough transform
        # Get first candidates for real lane lines  
        line_arr = self.hough_lines(canny_img, 1, 1 * np.pi/180, 30, 10, 20)
        ...
        # interpolation & collecting points for RANSAC
        L_interp = self.Collect_points(L_lines)
        R_interp = self.Collect_points(R_lines)

        #draw_circle(img,L_interp,(255,255,0))
        #draw_circle(img,R_interp,(0,255,255))

        # erase outliers based on best line
        self.left_fit_line = self.ransac_line_fitting(img, L_interp)
        self.right_fit_line = self.ransac_line_fitting(img, R_interp)

        # smoothing by using previous frames
        self.L_lane.append(sekf.left_fit_line), self.R_lane.append(self.right_fit_line)

        if len(self.L_lane) > 10:
            self.left_fit_line = self.smoothing(L_lane, 10)    
        if len(self.R_lane) > 10:
            self.right_fit_line = self.smoothing(R_lane, 10)
        final = self.draw_fitline(img, self.left_fit_line, self.right_fit_line)
    ...
    def start_line_detection(self):
        while(self.cap.isOpened()):
            try:
                ...
                result = self.detect_lanes_img(frame)
